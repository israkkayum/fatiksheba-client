...
const ProfilePicChange = (props) => {
  const { successMessage, tempAvatar, profile, loading, avatar } = props;
  ...
  return (
    ...
    >
      <div>
        ...
        <form
          onSubmit={props.handleRequestSubmit}
          ...
          {loading ? (
            ...
            <>
              {tempAvatar && avatar && !successMessage ? (
                <button
                  type="submit"
                  class="px-4 py-1 text-sm text-indigo-600 font-semibold rounded-md border border-indigo-200 hover:text-white hover:bg-indigo-600 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:ring-offset-2 mt-3"
                >
                  Submit
                </button>
              ) : (
                ...
                >
                  <input
                    ...
                    accept="image/*"
                    required
                    onChange={(e) => props.handleOnChange(e.target.files[0])}
                  />
                  <span> Change </span>
                </label>
              )}
